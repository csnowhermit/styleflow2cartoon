...
from .optimization import ExponentialMovingAverage
...
class UgatitSadalinHourglass(object):
    def __init__(self, args):
        ...
        self.device = f'cuda:{args.gpu_ids[0]}' if torch.cuda.is_available() else 'cpu'
        ...
        # 最后区分下平台
        self.platform = args.use_platform    # local / colab
    ...
    def build_model(self, mobile_facenet_path):
        """ DataLoader """
        ...
        if self.platform == "local":
            ...
            # colab中用这个
            self.trainA = ImageFolder(os.path.join('/content/drive/MyDrive/app/photo2cartoon/dataset/', self.dataset, 'trainA'), train_transform)
            self.trainB = ImageFolder(os.path.join('/content/drive/MyDrive/app/photo2cartoon/dataset/', self.dataset, 'trainB'), train_transform)
            self.testA = ImageFolder(os.path.join('/content/drive/MyDrive/app/photo2cartoon/dataset/', self.dataset, 'testA'), test_transform)
            self.testB = ImageFolder(os.path.join('/content/drive/MyDrive/app/photo2cartoon/dataset/', self.dataset, 'testB'), test_transform)
        ...
        self.facenet = FaceFeatures(mobile_facenet_path, self.device)
    ...
    def train(self):
        ema_genA2B = ExponentialMovingAverage(self.genA2B, 0.999)
        ema_genB2A = ExponentialMovingAverage(self.genB2A, 0.999)
        ema_disGA = ExponentialMovingAverage(self.disGA, 0.999)
        ema_disGB = ExponentialMovingAverage(self.disGB, 0.999)
        ema_disLA = ExponentialMovingAverage(self.disLA, 0.999)
        ema_disLB = ExponentialMovingAverage(self.disLB, 0.999)
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.print_freq == 0:
                ...
                # eval之前，调用apply_ema_weights
                ema_genA2B.apply_ema_weights()
                ema_genB2A.apply_ema_weights()
                ema_disGA.apply_ema_weights()
                ema_disGB.apply_ema_weights()
                ema_disLA.apply_ema_weights()
                ema_disLB.apply_ema_weights()
                ...
                display(Image.open(os.path.join(self.result_dir, self.dataset, 'img', 'A2B_%07d.png' % step)))
                display(Image.open(os.path.join(self.result_dir, self.dataset, 'img', 'B2A_%07d.png' % step)))

                # eval后，恢复到原来的参数
                ema_genA2B.reset_old_weights()
                ema_genB2A.reset_old_weights()
                ema_disGA.reset_old_weights()
                ema_disGB.reset_old_weights()
                ema_disLA.reset_old_weights()
                ema_disLB.reset_old_weights()
