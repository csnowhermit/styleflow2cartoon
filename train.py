...
def parse_args():
    ...
    parser.add_argument('--mobile_facenet_path', type=str, default='', help='mobile_facenet_path')
    parser.add_argument('--use_platform', type=str, default='local', help='which platform: [local / colab]')

    args = parser.parse_args()
    if args.use_platform == "local":    # 区分本地还是colab
        ...
        args.result_dir = '/content/drive/MyDrive/app/photo2cartoon/experiment/{}-size{}-ch{}-{}-lr{}-adv{}-cyc{}-id{}-identity{}-cam{}'.format(
            os.path.basename(__file__)[:-3],
            args.img_size,
            args.ch,
            args.light,
...
def main():
    ...
    print("use device: %s" % ("GPU" if torch.cuda.is_available() else "CPU"))
    gan = UgatitSadalinHourglass(args)
    gan.build_model(args.mobile_facenet_path)
    print("type(gan):", type(gan))
