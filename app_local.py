...
def generate_image(model, w, noise_mode='const'):
    ...
    # 这里使用style-mix的方式
    if torch.cuda.is_available():
        img = model.synthesis(torch.Tensor(w), noise_mode=noise_mode)  # w, [1, 18, 512]
    else:
...
def main():
    ...
    with st.spinner("Setting up... This might take a few minutes... Please wait!"):
        ...
        stylegan2_model, flow_model = init_model()    # stylegan2另起个服务跑
    ...
    img_source = generate_image(stylegan2_model, w_selected)
    ...
    with col2:
        ...
        img_target = generate_image(stylegan2_model, st.state.w_current)
